  ...
  <UButton
    ...
    :class="
      !areRequiredFieldsValid()
        ? ''
        : 'hover:shadow-md hover:shadow-green-600/50 hover:scale-105 active:scale-100 transition-all duration-150 ease-in-out'
    "
...
export default {
  ...
  data() {
    return {
      gradientStyle: {
        background: "linear-gradient(135deg, #00C16A, #00DC82)",
      },
    };
  },
  methods: {
    moveGradient(event) {
      if (this.areRequiredFieldsValid()) {
        ...
        this.gradientStyle.background = `radial-gradient(circle at ${x}% ${y}%, #00DC82, #00C16A)`;
      }
    },
    leaveGradient() {
      if (this.areRequiredFieldsValid()) {
        this.gradientStyle.background =
          "linear-gradient(135deg, #00C16A, #00DC82)";
      }
    },
    areRequiredFieldsValid() {
